...
| `apps/roadmap-site` — `/demo` | ✅ Pass | 2026-07-04 | Rewritten from a hardcoded fake simulation to fully live; sitewide dark mode fixed; Components section added and verified. Rechecked 2026-07-05 for layout-class/Tailwind `!important` collisions after the Layout module landed — none (no `data-ds-live` element carries a Tailwind responsive prefix). |
| `apps/roadmap-site` — `/docs` | ✅ Pass | 2026-07-05 | Session 23 rewrite needed two fix rounds (wrong hex values, stale count, hardcoded-fallback, disconnected dark toggle). Sessions 25–29 (Dark/Light rework + Layout section) reviewed: one bug — Usage example referenced nonexistent `.mx-auto` and was semantically broken (`max-w-screen` can't center) — fixed. Live demos verified via `getComputedStyle`. (The `.ease-ease-in-out` naming quirk this row used to reference was resolved 2026-07-18 — see the Motion easing rename row below.) |
