...
| Foundational architecture bug, present since initial commit, sitewide blast radius | 2 | `@layer` order put `utilities` before `components`; base `:root` never set a text `color` at all (light mode worked by accident, dark mode was unreadable) |

**Total distinct defects: ~30**, across roughly 94 Kiro session log entries.
...
- **Fixed by:** Claude, swapped the order to `reset, base, components, utilities, theme` in `assembler.ts` (declaration string + section-assembly order/comments), updated the one test asserting the old order and two doc mentions. Verified three ways: rebuilt `dist/core.css` (byte order now correct), full test suite 75/75 still green, and a live browser check confirming `#nav-toggle`'s computed `display` flips to `none` at 1280px post-fix. Merged to `master` via PR #4 (`69b265e`); jsdelivr CDN cache purged to match.

### 30. Base text `color` never set anywhere — `packages/css-core/lib/generators/base.ts` — light mode worked by accident, dark mode was unreadable
- **What happened:** `generateBase()`'s `:root` rule set `font-family`/`font-size`/`line-height` but **never set a `color`**, and no other generator — light theme, dark theme, or otherwise — set one either. Component classes (`.card`, `.badge`, `.btn`, etc.) each explicitly set their own `color: var(--ds-color-*-contrast)`, so text *inside* those always rendered correctly; but any plain heading or paragraph placed directly in a page, without such a wrapper, silently inherited the browser's hardcoded default black.
- **Impact:** invisible in light mode by pure accident (black text defaults to reading fine on a white page), but completely unreadable in dark mode — `getComputedStyle` on a bare `<h1>`/`<p>` returned `rgb(0, 0, 0)` on a `#121212` background. This is the second foundational, sitewide-blast-radius bug found in as many sessions (see #29) — both are the kind of defect that no build/test/lint/type-check gate in this repo can catch, because none of them evaluate rendered text contrast; only visually testing a plain, unwrapped element in dark mode reveals it.
- **How it surfaced:** while live-testing a dark-mode toggle added to an external test project (`DS-SYS-TEST_UI/html/index.html`) for an unrelated request (adding UI polish/flourishes) — not from any planned dark-mode audit.
- **Fixed by:** Claude, added `color: var(--ds-color-surface-contrast);` to the `:root` rule in `generateBase()` — reusing the exact token every component already relies on, so both themes resolve correctly with no new special-casing. Verified: `pnpm build`/`test` (75/75)/`lint` clean, and a live scratch-HTML check confirmed a bare `<h1>`/`<p>` computes to `#E6E1E5` on `#121212` in dark mode and `#1A1A1A` on white in light mode — exact matches to `dist/tokens.css`/`dist/themes/dark.css`. Not yet merged to `master`/CDN at time of writing — fixed locally on `features/dev` only.
...
7. **A foundational architectural choice was never stress-tested against its own stated purpose.** Item #29 — the `@layer` order — shipped in the initial commit and was never wrong in the sense of "doesn't build" or "doesn't match a spec doc"; it just quietly defeated the one thing utility classes are for (overriding components) on every page, for as long as the project existed. Item #30 — no base text `color` — is the same pattern from a different angle: it worked by pure accident in light mode, so nothing ever flagged it, until dark mode made the omission visible. Both were found the same way: not a planned audit, but live-testing an unrelated feature (a responsive nav, a dark-mode toggle) in a real browser and noticing something looked wrong. Every other item in this log involves a session doing something and getting it wrong; these two involve a decision nobody ever revisited once made, because none of the build/test/lint/type-check gates in this repo are capable of evaluating cascade-layer precedence or rendered text contrast — only a live browser exercising the exact combination (component + same-property utility; a plain element + dark theme) would ever reveal them, and neither combination apparently got checked until this session.
